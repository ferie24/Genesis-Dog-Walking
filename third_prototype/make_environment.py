...
class Go2WalkingEnv:
    def __init__(
        ...
        self.target_dof_pos = torch.zeros(
            (self.num_envs, self.num_actions), device=self.device
        )
        self._gravity_vec = torch.tensor([0.0, 0.0, -1.0], device=self.device).repeat(
            self.num_envs, 1
        )
        self._up_vec = torch.tensor([0.0, 0.0, 1.0], device=self.device).repeat(
            self.num_envs, 1
        )
        ...
        self.default_dof_pos = self.default_dof_pos.unsqueeze(0).repeat(
            self.num_envs, 1
        )
        ...
        self.reward_fn = (
            ...
            else lambda obs, actions, info: torch.zeros(
    ...
    def _setup_robot(self):
        ...
        for name in self.dof_names:
            ...
            if isinstance(idx_local, (list, tuple)) and len(idx_local) == 1:
                ...
                raise RuntimeError(
                    f"Unexpected dof index format for joint {name}: {idx_local}"
                )
    ...
    def step(self, actions):
        ...
        heading_diag = {
            "heading_error_abs_mean": heading_error.abs().mean(),
            "heading_error_signed_mean": heading_error.mean(),
            "heading_error_abs_max": heading_error.abs().max(),
        }
    ...
    def _compute_foot_diagnostics(self):
        ...
        diagonal_support = (fl & rr & ~fr & ~rl) | (fr & rl & ~fl & ~rr)
    ...
    def _sample_commands(self, env_ids):
        ...
        ).uniform_(*self.command_range["lin_vel_x"])
        self.commands[env_ids, 1] = torch.empty(
            len(env_ids), device=self.device
        ).uniform_(*self.command_range["lin_vel_y"])
        self.commands[env_ids, 2] = torch.empty(
            len(env_ids), device=self.device
        ).uniform_(*self.command_range["ang_vel_yaw"])
